...
class HIA_predict(torch.nn.Module):
    def __init__(self, emb_dim, dataset):
        ...
        elif self.dataset == "PMT":
            self.mlp = torch.nn.Sequential(torch.nn.Linear(emb_dim, 4*emb_dim), torch.nn.ReLU(), torch.nn.Linear(4*emb_dim, emb_dim), torch.nn.ReLU())
            self.pred_linear = torch.nn.Linear(self.emb_dim, 1)
